...
export default function MoPhanPage() {
    ...
    const handleDelete = async (moPhan: MoPhan) => {
        if (!window.confirm('Xoa thong tin mo phan nay?')) return;

        const res = await moPhanApi.delete(moPhan.id);
        if (res.success) {
            toast.success(res.message || 'Da xoa mo phan');
            await loadData();
        }
    };
    ...
    const copyLocation = async (moPhan: MoPhan) => {
        ...
        toast.success('Da sao chep toa do');
    };

    return (
        <AuthenticatedLayout>
            <Head title="Mo phan gia tien" />
            ...
            <div style={{ maxWidth: 1220, margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
                    <div>
                        ...
                        <h1 style={{ fontSize: 28, fontWeight: 700, color: 'var(--ink)', margin: '0 0 8px', fontFamily: 'Cormorant Garamond, serif' }}>
                            Toa do mo phan gia tien
                        </h1>
                        <p style={{ fontSize: 13.5, color: 'var(--ink-mute)', margin: 0, maxWidth: 720 }}>
                            Luu toa do GPS va ghi chu duong di de con chau trong dong ho tim mo phan de dang hon.
                        </p>
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12, marginBottom: 20 }}>
                    <StatCard label="Nguoi da mat" value={deceasedMembers.length} icon="users" color="brown" />
                    <StatCard label="Da co toa do" value={withLocation} icon="pin" color="jade" />
                    <StatCard label="Chua co toa do" value={withoutLocation} icon="map" color="terracotta" />
                </div>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 18 }}>
                    <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
                        ...
                        <input
                            ...
                            placeholder="Tim theo ten nguoi da mat..."
                    ...
                    >
                        <option value="">Tat ca nguoi da mat</option>
                ...
                {loading ? (
                    <div style={{ display: 'grid', placeItems: 'center', height: 280 }}>
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: 13, color: 'var(--ink-mute)' }}>Dang tai du lieu mo phan...</div>
                        </div>
                    </div>
                ) : rows.length === 0 ? (
                    <div style={{ background: 'var(--bg-elev)', borderRadius: 16, border: '1px solid var(--line)', padding: '54px 24px', textAlign: 'center' }}>
                        ...
                        <div style={{ marginTop: 12, fontSize: 15, fontWeight: 700, color: 'var(--ink-mute)' }}>Chua co du lieu phu hop</div>
                    </div>
                ) : (
        ...
        <div style={{ background: 'var(--bg-elev)', borderRadius: 16, border: '1px solid var(--line)', padding: 16, boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
                <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--ink-mute)', marginTop: 2 }}>
                        Doi {member.doi_thu || '-'} - Mat ngay {member.ngay_mat || 'chua ro'}
                    </div>
                </div>
            ...
            {moPhan ? (
                <>
                    <div style={{ borderRadius: 12, background: 'var(--card-soft)', border: '1px solid var(--line-soft)', padding: 12 }}>
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                            <CoordinatePill label="Vi do" value={Number(moPhan.vi_do).toFixed(7)} />
                            <CoordinatePill label="Kinh do" value={Number(moPhan.kinh_do).toFixed(7)} />
                        </div>
                        <div style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.55 }}>
                            {moPhan.ghi_chu || 'Chua co ghi chu vi tri.'}
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', borderTop: '1px solid var(--line-soft)', paddingTop: 12 }}>
                        <div style={{ fontSize: 11.5, color: 'var(--ink-mute)' }}>
                            Cap nhat: {formatDateTime(moPhan.updated_at)}
                        </div>
                        <div style={{ display: 'flex', gap: 6 }}>
                            <button type="button" onClick={onUpdate} className="gp-btn gp-btn-ghost" title="Cap nhat mo phan">
                                <Icon name="edit" size={14} />
                            </button>
                            {canDelete && (
                                <button type="button" onClick={onDelete} className="gp-btn gp-btn-ghost" title="Xoa mo phan" style={{ color: 'var(--crimson)' }}>
                ...
                <div style={{ borderRadius: 12, background: 'color-mix(in srgb, var(--terracotta) 8%, transparent)', border: '1px dashed color-mix(in srgb, var(--terracotta) 35%, transparent)', padding: 14 }}>
                    <div style={{ fontSize: 13, color: 'var(--ink-soft)', marginBottom: 12 }}>Chua co toa do mo phan cho thanh vien nay.</div>
...
function MoPhanFormModal({
    ...
    const useCurrentLocation = () => {
        if (!navigator.geolocation) {
            setError('Thiet bi khong ho tro lay vi tri GPS.');
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            () => {
                setError('Khong the lay vi tri hien tai. Vui long cap quyen GPS hoac nhap thu cong.');
                setGpsLoading(false);
            },
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        if (!editing && !form.thanh_vien_id) {
            setError('Vui long chon thanh vien da mat.');
            return;
        }

        if (!Number.isFinite(viDo) || viDo < -90 || viDo > 90) {
            setError('Vi do phai nam trong khoang -90 den 90.');
            return;
        }

        if (!Number.isFinite(kinhDo) || kinhDo < -180 || kinhDo > 180) {
            setError('Kinh do phai nam trong khoang -180 den 180.');
            return;
        }
        ...
        try {
            ...
            if (res.success) {
                toast.success(res.message || 'Da luu mo phan');
                await onSaved();
            } else {
                setError(res.message || 'Khong the luu mo phan.');
            }
        } finally {
            setSaving(false);
        }
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'grid', placeItems: 'center', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)', padding: 16 }}>
            <form onSubmit={handleSubmit} style={{ width: '100%', maxWidth: 560, background: 'var(--bg-elev)', borderRadius: 18, border: '1px solid var(--line)', boxShadow: '0 24px 60px rgba(0,0,0,0.28)', overflow: 'hidden' }}>
                <div style={{ padding: '18px 22px', background: 'linear-gradient(135deg, var(--gold), var(--brown-soft))', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                    <div>
                        <div style={{ fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', fontWeight: 700, opacity: 0.8 }}>Mo phan</div>
                        <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{editing ? 'Cap nhat toa do' : 'Them toa do mo phan'}</h2>
                    </div>
                ...
                <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        <span style={{ fontSize: 12.5, color: 'var(--ink-soft)', fontWeight: 700 }}>Thanh vien da mat</span>
                    ...
                    <button type="button" onClick={useCurrentLocation} disabled={gpsLoading} className="gp-btn gp-btn-ghost" style={{ justifyContent: 'center', opacity: gpsLoading ? 0.65 : 1 }}>
                        ...
                        {gpsLoading ? 'Dang lay vi tri...' : 'Lay vi tri GPS hien tai'}
                    </button>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <span style={{ fontSize: 12.5, color: 'var(--ink-soft)', fontWeight: 700 }}>Vi do</span>
                        ...
                        <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <span style={{ fontSize: 12.5, color: 'var(--ink-soft)', fontWeight: 700 }}>Kinh do</span>
                    ...
                    <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        <span style={{ fontSize: 12.5, color: 'var(--ink-soft)', fontWeight: 700 }}>Ghi chu tim duong</span>
                        <textarea value={form.ghi_chu} onChange={(event) => setField('ghi_chu', event.target.value)} className="gp-input" rows={4} placeholder="Vi du: Mo nam canh cay dua to, hang thu hai ben trai..." style={{ resize: 'vertical' }} />
                    </label>

                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 6 }}>
                        <button type="button" onClick={onClose} className="gp-btn gp-btn-ghost">Huy</button>
                        <button type="submit" disabled={saving} className="gp-btn gp-btn-primary" style={{ opacity: saving ? 0.65 : 1 }}>
                            {saving ? 'Dang luu...' : 'Luu mo phan'}
                        </button>
                    </div>
                </div>
            </form>
